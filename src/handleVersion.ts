...
export async function handleVersion(
  ...
  if (version !== 'SNAPSHOT') {
    ...
    if (!/^\d+\.\d+\.\d+$/.test(version)) {
      throw new Error(`Version '${version}' is not valid SemVer. Aborting git commit.`)
    }
    ...
    try {
      await exec('git', ['commit', '-m', `github-actions[bot] version.json updated with image version ${version}`])
    } catch (e) {
